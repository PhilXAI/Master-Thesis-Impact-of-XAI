...
if st.session_state.current != None:
    if st.session_state.current == "A":
        ...
        st.write("My Guess is: **Jerusalem**. In particular, the colored areas below have helped me for my guess.")
        image=Image.open("Bild8L.png")
        st.image(image)
        ...
        if T:
            ...
            st.markdown('<a href="/Round_9" target="_self">Next Round</a>', unsafe_allow_html=True)
        if J:
